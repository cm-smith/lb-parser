...
#include <stdlib.h>                          // free, calloc, atoi
#include <string.h>                          // strlen, strcpy
#include <dirent.h>                          // scandir, alphasort
#include <sys/stat.h>                        // stat functionality
#include <fcntl.h>			     // open
#include <stdio.h>			     // file functionality
...
{
	// cast filename from string to int
	int id = atoi(fileName);
	
	// if the casting was unsuccessful, id == 0
	return id;
}

/* LoadDocument - gets all the contents off the given file and puts in buffer
 * @fileName: the path to file to load contents from
 * Returns a pointer to the buffer containing contents from file
 */
char *LoadDocument(char *fileName)
{
	/* revised code from 
	 * http://www.go4expert.com/articles/understanding-linux-fstat-example-t27449/
	 * by user poornaMoksha on 26 December, 2011
	 */
	struct stat sb;
	int f_d = open(fileName, O_RDONLY);
	
	// set errno to default value
	errno = 0;

	// if the stat function retrieved nothing, don't go any further
	if(-1 == f_d) {
		return NULL;
	}

	// if stat runs into errors, don't return anything
	if (fstat(f_d, &sb)) {
		return NULL;
	}
	
	//printf("The size of the file is |%zd|\n", sb.st_size);
	char *res = (char *)malloc(sb.st_size+1);
	memset(res, 0, sb.st_size+1);

	FILE *file = fopen(fileName, "r");
	
	// read all the contents off the file
	fread(res, sb.st_size+1, 1, file);
